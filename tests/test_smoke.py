...
def test_predict_endpoint_quali_freq(api_client, fixture_dir, tmp_path):
    ...
    shutil.copy(
        fixture_dir / "data" / "features.parquet",
    ...
    models_dir.mkdir()
    shutil.copy(
        fixture_dir / "models" / "quali_freq.joblib",
        models_dir / "quali_freq.joblib"
    )
    ...
    try:
        ...
        print(f"✓ Predict endpoint works with quali_freq")
...
def test_backtest_script_on_fixtures(fixture_dir, tmp_path):
    ...
    cmd = [
        ...
        "--data", str(fixture_dir / "data" / "features.parquet"),
        "--models", "quali_freq",  # Only test one model for speed
        "--output", str(output_dir),
        "--min-train-races", "2"  # Use minimal training for fixtures
    ]
    ...
    print(f"✓ Backtest script generates reports")
