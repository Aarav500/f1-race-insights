...
from f1.models.baselines import QualifyingFrequencyBaseline, EloBaseline
...
def test_qualifying_baseline_determinism():
    ...
    assert model1.podium_probs == model2.podium_probs, "Podium probabilities should be deterministic"
...
def test_elo_baseline_determinism():
    ...
    assert model1.constructor_ratings == model2.constructor_ratings, "Constructor ratings should be deterministic"
...
def test_elo_baseline_probabilities():
    ...
    for race_id in predictions['race_id'].unique():
        ...
        assert abs(win_prob_sum - 1.0) < 0.01, f"Win probabilities should sum to 1, got {win_prob_sum}"
...
def test_qualifying_baseline_empirical_accuracy():
    ...
    assert model.podium_probs[2] == 1.0, "Position 2 should have 100% podium rate (always finishes top 3)"
...
def test_elo_rating_changes():
    ...
    assert ver_rating > ham_rating, f"VER ({ver_rating}) should have higher rating than HAM ({ham_rating})"
