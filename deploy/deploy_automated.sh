...
sleep 3
if $SSH_CMD "curl -s -o /dev/null -w '%{http_code}' http://localhost:3000/f1-insights/" | grep -q "200"; then
    success "Application responding on port 3000"
else
    error "Application health check failed"
fi

if $SSH_CMD "curl -s -o /dev/null -w '%{http_code}' http://localhost/f1-insights/" | grep -q "200\|301\|302"; then
