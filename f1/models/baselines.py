...
class EloBaseline(BaselineModel):
    ...
    def _update_ratings(self, race_results: pd.DataFrame) -> None:
        ...
        for i, (idx_i, driver_i) in enumerate(results.iterrows()):
            ...
            for j, (idx_j, driver_j) in enumerate(results.iterrows()):
                ...
                # Expected scores
                driver_expected = self._expected_score(driver_rating_i, self._get_driver_rating(driver_id_j))
                constructor_expected = self._expected_score(constructor_rating_i, self._get_constructor_rating(team_j))
            ...
            new_driver_ratings[driver_id_i] = driver_rating_i + (self.k_factor * driver_rating_change) / (n_drivers - 1)
            new_constructor_ratings[team_i] = constructor_rating_i + (self.k_factor * constructor_rating_change) / (n_drivers - 1)
    ...
    def fit(self, train_df: pd.DataFrame) -> 'EloBaseline':
        ...
        logger.info(f"Tracking {len(self.driver_ratings)} drivers, {len(self.constructor_ratings)} constructors")
        ...
        for race_id, race_data in df.groupby('race_id'):
            ...
            for i, (_, row) in enumerate(race_data.iterrows()):
                predictions.append({
                    ...
                    'constructor_rating': self._get_constructor_rating(row.get('team', 'Unknown'))
                })
