...
def create_pairwise_comparisons(
    ...
    for race_id, race_data in features_df.groupby('race_id'):
        ...
        if not include_dnf:
            ...
            race_data = race_data[
                race_data['finish_position'].notna() & 
                (race_data['finish_position'] <= 20)
            ]
        ...
        for i in range(n_drivers):
            for j in range(i + 1, n_drivers):  # Only generate each pair once (i < j in index)
                ...
                if pd.notna(finish_i) and pd.notna(finish_j):
                    ...
                    # Build pairwise row
                    pair_row = create_pair_features(
                        driver_i, driver_j, y_ij, race_id, race_data
                    )
                    pairwise_data.append(pair_row)
    
    pairwise_df = pd.DataFrame(pairwise_data)
    logger.info(f"Created {len(pairwise_df)} pairwise comparisons from {features_df['race_id'].nunique()} races")
