...
class PairwiseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.numeric_feature_cols:
            feats_i = torch.tensor([row.get(f'{col}_i', 0.0) for col in self.numeric_feature_cols], dtype=torch.float32)
            feats_j = torch.tensor([row.get(f'{col}_j', 0.0) for col in self.numeric_feature_cols], dtype=torch.float32)
        else:
            feats_i = None
            feats_j = None
...
class NBTTLFTrainer:
    ...
    def train_epoch(self, train_loader: DataLoader) -> float:
        ...
        for batch in train_loader:
            ...
            features_i = batch['features_i'].to(self.device) if batch['features_i'] is not None else None
            features_j = batch['features_j'].to(self.device) if batch['features_j'] is not None else None
            
            # Forward pass
            self.optimizer.zero_grad()
            y_pred = self.model(
                driver_i, constructor_i, driver_j, constructor_j,
                track, race_idx, features_i, features_j
            )
