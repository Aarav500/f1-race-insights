...
def apply_deltas(
    ...
    if 'driver_form_delta' in changes:
        ...
        for col in ['driver_rolling_avg_finish', 'driver_rolling_avg_points', 'driver_rolling_dnf_rate']:
    ...
    if 'constructor_form_delta' in changes:
        ...
        for col in ['constructor_rolling_avg_finish', 'constructor_rolling_avg_points', 'constructor_rolling_dnf_rate']:
            if col in modified_data.columns:
                current = modified_data.loc[driver_mask, col].values[0]
                if 'dnf_rate' in col:
                    new_val = current * (1 - delta)
...
def compute_counterfactual(
    ...
    logger.info(f"Counterfactual delta: win_prob {delta['win_prob']:+.3f}, "
