...
def extract_qualifying_features(qual_results: pd.DataFrame) -> pd.DataFrame:
    ...
    # Qualifying position
    features = df[['race_id', 'year', 'round', 'Abbreviation', 'Position', 'TeamName', 'best_quali_time']].copy()
...
def extract_race_features(race_results: pd.DataFrame) -> pd.DataFrame:
    ...
    features = df[['race_id', 'year', 'round', 'Abbreviation', 'Position', 
                    'Status', 'Points', 'race_name', 'country']].copy()
    features.rename(columns={
        'Abbreviation': 'driver_id',
...
def compute_rolling_driver_form(
    ...
    race_order = df[['year', 'round']].drop_duplicates().sort_values(['year', 'round']).reset_index(drop=True)
...
def compute_rolling_constructor_form(
    ...
    for (year, round_num), race_data in df.groupby(['year', 'round']):
        ...
        for team in race_data['team'].unique():
            ...
            prior_team_races = df[
                (df['team'] == team) & 
                (df['race_index'] < current_race_idx)
...
def build_feature_table(
    ...
    if 'team_race' in race_features_with_team.columns:
        race_features_with_team['team'] = race_features_with_team['team'].fillna(race_features_with_team['team_race'])
    ...
    constructor_rolling = compute_rolling_constructor_form(race_features_with_team, window=rolling_window)
    
    # Merge all features
    features = qual_features.merge(
        driver_rolling,
    ...
    features = features.merge(
        constructor_rolling[['race_id', 'driver_id', 'constructor_rolling_avg_finish', 
                             'constructor_rolling_avg_points', 'constructor_rolling_dnf_rate',
    ...
    features = features.merge(
        track_info[['year', 'round', 'track_id']],
        on=['year', 'round'],
        how='left'
    )
    ...
    final_cols = [
        'race_id', 'season', 'round', 'track_id', 'driver_id', 'team',
        'quali_position', 'quali_delta_to_pole', 'quali_delta_to_teammate',
        'driver_rolling_avg_finish', 'driver_rolling_avg_points', 'driver_rolling_dnf_rate',
        'constructor_rolling_avg_finish', 'constructor_rolling_avg_points', 'constructor_rolling_dnf_rate'
    ]
