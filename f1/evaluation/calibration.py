...
def calibrate_tree_model_predictions(
    ...
    if 'race_id' in calibrated.columns:
        calibrated = calibrated.groupby('race_id', group_keys=False).apply(
            lambda x: normalize_race_probabilities(x, ['win_prob', 'podium_prob'])
    ...
    if validation_df is not None and method != 'none':
        ...
        if 'finish_position' in validation_df.columns:
            ...
            if 'race_id' in val_with_probs.columns:
                val_with_probs = val_with_probs.groupby('race_id', group_keys=False).apply(
                    softmax_within_race
                ).reset_index(drop=True)
            ...
            calibrated['podium_prob'] = podium_calibrator.transform(calibrated['podium_prob_raw'].values)
        else:
