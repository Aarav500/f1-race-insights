...
class WalkForwardBacktest:
    ...
    def run(
        ...
        if len(race_dates) < self.min_train_races + 1:
            raise ValueError(
                f"Insufficient data: {len(race_dates)} races < "
                f"{self.min_train_races + 1} required"
            )
        ...
        if hasattr(model, 'predict'):
            ...
            if isinstance(predictions, pd.DataFrame):
                if task == 'win':
                    ...
                    return predictions.get('expected_finish', predictions['finish_position']).tolist()
            else:
        ...
        if model_type == 'quali_freq':
            ...
            class ModelWrapper:
                ...
                def predict(self, data):
                    ...
                    probs = self.model.predict_proba(X) if hasattr(self.model, 'predict_proba') else self.model.predict(X)
